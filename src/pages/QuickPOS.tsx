...
export default function QuickPOS() {
  ...
  return (
    <div className="w-screen h-screen flex flex-col bg-gray-50 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col lg:flex-row min-h-0 min-w-0 overflow-hidden">
        ...
        <div className="bg-blue-800 text-white w-full lg:w-48 flex flex-col flex-shrink-0 min-h-0 lg:min-h-0">
          ...
          <div className="p-2 sm:p-3 border-t border-blue-700">
            <button 
              onClick={logout}
              className="w-full flex items-center justify-center gap-2 px-2 sm:px-3 py-1 sm:py-2 text-white hover:bg-red-600 rounded transition-colors text-xs sm:text-sm"
            >
                  ...
                  {cart.items.length === 0 ? (
                    ...
                    <tr key={item.product.id} className="border-b">
                      ...
                      <td className="text-center">
                        <button onClick={() => cart.removeItem(item.product.id)} className="text-red-600 hover:text-red-800"><X className="w-4 h-4 sm:w-5 sm:h-5" /></button>
                      </td>
                    </tr>
                  ))}
                </tbody>
      ...
      {isPaymentDialogOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg p-4 sm:p-8 w-full max-w-2xl shadow-lg relative flex flex-col lg:flex-row gap-4 sm:gap-8">
            ...
            <div className="flex-1 min-w-[260px]">
              <h2 className="text-lg sm:text-xl font-bold mb-4">{paymentMethod === 'cash' ? 'Cash Payment' : paymentMethod === 'card' ? 'Card Payment' : 'Wallet Payment'}</h2>
              <div className="mb-4">
                <label className="block text-gray-700 font-semibold mb-1 text-sm sm:text-base">Bill Amount</label>
                <input type="text" className="border rounded px-3 py-2 w-full bg-gray-100 text-sm sm:text-base" value={`₹${cart.getTotal().toFixed(2)}`} readOnly />
              </div>
              {paymentMethod === 'cash' && (
                <>
                  <div className="mb-4">
                    <label className="block text-gray-700 font-semibold mb-1 text-sm sm:text-base">Tendered <span className="text-red-600">*</span></label>
                    <input
                      type="text"
                      className="border rounded px-3 py-2 w-full text-right text-base sm:text-lg"
                      value={cashAmount}
                  ...
                  <div className="mb-4">
                    <label className="block text-gray-700 font-semibold mb-1 text-sm sm:text-base">Change Due</label>
                    <input type="text" className="border rounded px-3 py-2 w-full bg-gray-100 text-right text-sm sm:text-base" value={`₹${Math.max(0, (parseFloat(cashAmount) || 0) - cart.getTotal()).toFixed(2)}`} readOnly />
                    <div className="text-xs text-gray-500 mt-1">Net Amount Received : ₹{Math.min(cart.getTotal(), parseFloat(cashAmount) || 0).toFixed(2)}</div>
                  </div>
                </>
              )}
              {paymentMethod === 'card' && (
                <>
                  <div className="mb-4">
                    <label className="block text-gray-700 font-semibold mb-1 text-sm sm:text-base">Amount <span className="text-red-600">*</span></label>
                    <input
                      type="text"
                      className="border rounded px-3 py-2 w-full text-right text-base sm:text-lg"
                      value={cashAmount}
                  ...
                  <div className="mb-4">
                    <label className="block text-gray-700 font-semibold mb-1 text-sm sm:text-base">Txn. ID</label>
                    <input
                      type="text"
              ...
              <div className="flex gap-2 sm:gap-4 mt-6">
                <button className="flex-1 bg-gray-200 hover:bg-gray-300 text-gray-700 font-bold py-2 rounded text-sm sm:text-base" onClick={() => setIsPaymentDialogOpen(false)}>Cancel</button>
                <button className="flex-1 bg-blue-700 hover:bg-blue-800 text-white font-bold py-2 rounded flex items-center justify-center gap-2 text-sm sm:text-base" onClick={() => { handleTransactionComplete(); setIsPaymentDialogOpen(false); }}>
                  <svg width="16" height="16" className="sm:w-5 sm:h-5" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 17v-2a4 4 0 0 1 4-4h10"/><path d="M17 17v-2a4 4 0 0 0-4-4H3"/><circle cx="12" cy="7" r="4"/></svg>
                  Save n Print
                </button>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-2 items-center justify-center">
              <div className="grid grid-cols-4 gap-1 sm:gap-2 mb-2">
                {[2000, 500, 200, 100, 1,2,3,4,5,6,7,8,9,0,'.','CLR'].map((key, idx) => (
                  <button
                    key={idx}
                    className={`bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 sm:py-3 rounded text-sm sm:text-lg ${key==='CLR' ? 'col-span-2 bg-red-600 hover:bg-red-700' : ''}`}
